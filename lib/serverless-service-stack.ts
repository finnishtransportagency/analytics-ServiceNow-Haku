...
import { CfnOutput, Construct, StackProps, Stage, Tag } from '@aws-cdk/core';
...
export class ServerlessServiceStack extends cdk.Stack {
    constructor(scope: cdk.Construct, id: string, props?: StackProps) {
      ...
      { //DO NOT change this object, it will clear secrets, manually add secrets to it and query them from lambda. 
...
function datapipeServiceNowTable(construct:cdk.Construct,rajapintaName:string, appnameAndEnv:string ,secretmanager: secretsmanager.Secret,region:string,lambdaRole:iam.IRole,urlsecretHint:string,secretusernameHint:string,secretpasswordHint:string){
...
/*    
  const apiLambda=new lambda.Function(construct, 'APIFetch'+resourcenaming, {
    code: lambda.Code.fromAsset("./lambdas/api/path/", ),
